...
class LabelDialog(QDialog):
    ...
    def __init__(
        ...
        lane = self.schema.get_lane_by_level(self.level)
        self._is_notes_lane = bool(lane and lane.name.strip().lower() == "notes")
    ...
    def _setup_ui(self) -> None:
        """Create dialog UI."""
        lane = self.schema.get_lane_by_level(self.level)
        level_name = lane.name if lane else "Annotation"
        ...
        labels = list(lane.labels) if lane else []
        for label in labels:
