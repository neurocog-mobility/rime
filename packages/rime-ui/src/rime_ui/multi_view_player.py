...
class MultiViewPlayer(QWidget):
    ...
    Replaces VideoPlayer. The primary pane drives timing; secondary panes
    play in parallel (not seeked every frame).
    """
    ...
    def load_video(self, path: str) -> None:
        """Backward-compatible: load a single video."""
        vc = VideoConfig(path=path, role="primary")
        self.load_videos([vc], Path("/"))
