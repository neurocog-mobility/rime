...
from PySide6.QtGui import QPalette, QColor, QMouseEvent, QPainter, QPen
from PySide6.QtWidgets import QWidget

# Reused overview interaction, with neutral colors and no scientific services.
COLOR_BORDER = COLOR_OVERVIEW_HANDLE = COLOR_PLAYHEAD = "#555555"
COLOR_OVERVIEW_BG = COLOR_WINDOW_BG = "#eeeeee"
COLOR_OVERVIEW_VIEWPORT = "#dddddd"
COLOR_OVERVIEW_VIEWPORT_BORDER = COLOR_TIME_LABEL = "#000000"
COLOR_LOOP_ACCENT = COLOR_LOOP_BORDER = "#777777"
SIGNAL_PLOT_COLORS = ["#555555"]
...
class OverviewStrip(QWidget):
    ...
    def __init__(self, parent=None, *, embedded=False) -> None:
        ...
        self._activity_annotations: list[object] = []
        ...
        self.setMinimumHeight(18 if embedded else 72)
        self.setMaximumHeight(16777215 if embedded else 96)
        self.setMouseTracking(True)
        if embedded:
            self._MARGIN_X = 0
            self._WINDOW_HANDLE_WIDTH = 6
            self.setAccessibleName("Full recording zoom range")
            self.setToolTip(
                "Full recording · Drag edges to zoom · Drag the range to pan\n"
                "Click outside the range to seek · Double-click to fit"
            )
    ...
    def _color(self, value):
        if not getattr(self, "native_palette", False):
            return QColor(value)
        role = {
            "#eeeeee": QPalette.ColorRole.Window,
            "#dddddd": QPalette.ColorRole.Highlight,
            "#000000": QPalette.ColorRole.Text,
            "#555555": QPalette.ColorRole.Text,
            "#777777": QPalette.ColorRole.Highlight,
        }.get(value)
        return self.palette().color(role) if role is not None else QColor(value)
    ...
    def set_annotations(self, annotations: list[object]) -> None:
        self._activity_annotations = [
            annotation for annotation in annotations if not annotation.ghost
        ]
        self.update()
    ...
    def paintEvent(self, event) -> None:
        ...
        painter.fillRect(self.rect(), self._color(COLOR_WINDOW_BG))

        if self.embedded:
            self._paint_embedded(painter)
            painter.end()
            return

        strip_rect = self._strip_rect()
        painter.fillRect(strip_rect, self._color(COLOR_OVERVIEW_BG))

        # Subtle session ruler baseline.
        painter.setPen(QPen(self._color(COLOR_BORDER), 1))
        ...
        window_color = self._color(COLOR_OVERVIEW_VIEWPORT)
        ...
        painter.fillRect(
            ...
            strip_rect.height(),
            self._color(COLOR_OVERVIEW_HANDLE),
        )
        painter.fillRect(
        ...
        painter.setPen(QPen(self._color(COLOR_OVERVIEW_VIEWPORT_BORDER), 2))
        ...
        painter.setPen(QPen(self._color(COLOR_PLAYHEAD), 2))
        painter.drawLine(
            int(playhead_x), strip_rect.top() - 4, int(playhead_x), strip_rect.bottom() + 6
        )
        painter.setBrush(self._color(COLOR_PLAYHEAD))
        painter.drawEllipse(
            QPointF(playhead_x, strip_rect.top() - 4), self._PLAYHEAD_RADIUS, self._PLAYHEAD_RADIUS
        )
        self._draw_time_labels(painter, strip_rect)

        painter.end()

    def _paint_embedded(self, painter):
        rect = self._strip_rect()
        self._draw_annotation_minimap(painter, rect)
        self._draw_loop_region(painter, rect)
        left = self._time_to_x(self._view_start_ms, rect)
        right = self._time_to_x(self._view_end_ms, rect)
        color = self.palette().color(QPalette.ColorRole.Highlight)
        fill = QColor(color)
        fill.setAlpha(35)
        painter.fillRect(int(left), rect.top(), max(1, int(right - left)), rect.height(), fill)
        painter.setPen(QPen(color, 1))
        painter.drawRect(int(left), rect.top(), max(1, int(right - left) - 1), rect.height() - 1)
        for x in (left, right - self._WINDOW_HANDLE_WIDTH):
            painter.fillRect(int(x), rect.top(), self._WINDOW_HANDLE_WIDTH, rect.height(), color)
        playhead = self._time_to_x(self._position_ms, rect)
        painter.setPen(QPen(QColor("#ff4b55"), 2))
        painter.drawLine(int(playhead), rect.top(), int(playhead), rect.bottom())
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if self.embedded:
            # Zoom handles remain reachable even when the playhead is at an edge.
            distances = (abs(x - view_left), abs(x - view_right))
            if min(distances) <= self._EDGE_HIT_WIDTH:
                self._drag_mode = "resize_left" if distances[0] < distances[1] else "resize_right"
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if self._drag_mode == "resize_left":
            self.setCursor(Qt.CursorShape.SizeHorCursor)
            self._emit_view_range(
                max(0, min(self._drag_anchor_view_start_ms + delta_ms, self._drag_anchor_view_end_ms - min(500, self._duration_ms))),
        ...
        if self._drag_mode == "resize_right":
            ...
            self._emit_view_range(
                ...
                min(self._duration_ms, max(self._drag_anchor_view_end_ms + delta_ms, self._drag_anchor_view_start_ms + min(500, self._duration_ms))),
            )
            return
        ...
        view_right = self._time_to_x(self._view_end_ms, strip_rect)
        if (
            abs(x - view_left) <= self._EDGE_HIT_WIDTH
            or abs(x - view_right) <= self._EDGE_HIT_WIDTH
        ):
            self.setCursor(Qt.CursorShape.SizeHorCursor)
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        if self.embedded and event.button() == Qt.MouseButton.LeftButton and self._drag_mode:
    ...
    def mouseDoubleClickEvent(self, event):
        if self.embedded and event.button() == Qt.MouseButton.LeftButton:
            self._drag_mode = None
            self._emit_view_range(0, self._duration_ms)
            event.accept()
            return
    ...
    def _emit_position(self, x: float, strip_rect) -> None:
        ...
        if self.embedded and not self._view_start_ms <= position_ms <= self._view_end_ms:
            span = self._view_end_ms - self._view_start_ms
            self._emit_view_range(position_ms - span / 2, position_ms + span / 2)
    ...
    def _strip_rect(self):
        if self.embedded:
            return self.rect().adjusted(0, 2, -1, -2)
    ...
    def _time_to_x(self, time_ms: float, strip_rect) -> float:
        ...
        return (
            float(strip_rect.left())
            + (max(0.0, min(time_ms, self._duration_ms)) / self._duration_ms) * width
        )
    ...
    def _draw_time_labels(self, painter: QPainter, strip_rect) -> None:
        painter.setPen(QPen(self._color(COLOR_TIME_LABEL), 1))
    ...
    def _draw_annotation_minimap(self, painter: QPainter, strip_rect) -> None:
        ...
        for annotation in self._activity_annotations:
            ...
            color = (
                ...
                if self.embedded else QColor(SIGNAL_PLOT_COLORS[0])
            )
    ...
    def _draw_loop_region(self, painter: QPainter, strip_rect) -> None:
        if (
            self._loop_start_ms is None
            or self._loop_end_ms is None
            or self._loop_end_ms <= self._loop_start_ms
        ):
            return
        left = self._time_to_x(self._loop_start_ms, strip_rect)
        right = self._time_to_x(self._loop_end_ms, strip_rect)
        loop_fill = self._color(COLOR_LOOP_BORDER)
        ...
        painter.setPen(QPen(self._color(COLOR_LOOP_ACCENT), 1))
