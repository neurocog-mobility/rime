...
from rime_ui.panels.model_eval_panel import ModelEvalPanel
from rime_ui.panels.model_runner import ModelRunnerPanel
from rime_ui.widgets.annotation_toolbar import AnnotationToolbar
from rime_ui.widgets.multi_view_player import (
    MODE_PRIMARY_ONLY,
    MODE_SECONDARY_ONLY,
    MODE_SIDE_BY_SIDE,
    MultiViewPlayer,
...
from rime_ui.workflows.model_workflows import ModelWorkflowHelper
...
class PanelBinding:
    """Dock/action wiring for a toggleable side panel."""

    key: str
    mode: LayoutMode
    action: QAction
    dock: QDockWidget
    persist_visibility: bool
...
class RimeMainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self._panel_bindings_by_key: dict[str, PanelBinding] = {}
        self._model_workflows = ModelWorkflowHelper(self)
    ...
    def _setup_menu(self) -> None:
        ...
        self.annotation_list_action.toggled.connect(partial(self._on_toggle_panel, "annotation_list"))
        ...
        self.model_runner_action.toggled.connect(partial(self._on_toggle_panel, "model_runner"))
        ...
        self.model_eval_action.toggled.connect(partial(self._on_toggle_panel, "model_evaluation"))
        ...
        self.clinical_action.toggled.connect(partial(self._on_toggle_panel, "clinical_outcomes"))
        ...
        self.irr_action.toggled.connect(partial(self._on_toggle_panel, "irr_panel"))
    ...
    def _setup_central_widget(self) -> None:
        ...
        self.annotation_list_dock = self._create_right_dock(
            "Annotation List",
            self.annotation_list_panel,
            object_name="annotation_list_dock",
            panel_key="annotation_list",
        )
        ...
        self.model_runner_dock = self._create_right_dock(
            "Model Runner",
            self.model_runner_panel,
            object_name="model_runner_dock",
            panel_key="model_runner",
        )

        self.model_eval_panel = ModelEvalPanel(self)
        self.model_eval_dock = self._create_right_dock(
            "Model Evaluation",
            self.model_eval_panel,
            object_name="model_eval_dock",
            panel_key="model_evaluation",
        )

        self.clinical_panel = ClinicalPanel(self)
        self.clinical_dock = self._create_right_dock(
            "Clinical Outcomes",
            self.clinical_panel,
            object_name="clinical_dock",
            panel_key="clinical_outcomes",
        )
        ...
        self.irr_panel.matched_episode_store_changed.connect(self._on_matched_episode_store_changed)
        self.irr_panel.close_requested.connect(self._on_close_comparison)
        self.irr_dock = self._create_right_dock(
            "IRR Panel",
            self.irr_panel,
            object_name="irr_dock",
            panel_key="irr_panel",
            closable=False,
        )
        self._initialize_panel_bindings()
    ...
    def _create_right_dock(
        ...
        *,
        ...
        dock.visibilityChanged.connect(partial(self._on_panel_dock_visibility_changed, panel_key))
        dock.topLevelChanged.connect(self._on_dock_layout_changed)
        dock.dockLocationChanged.connect(self._on_dock_layout_changed)
        if hasattr(self, "annotation_list_dock") and dock is not self.annotation_list_dock:
            self.tabifyDockWidget(self.annotation_list_dock, dock)
        return dock

    def _initialize_panel_bindings(self) -> None:
        self._panel_bindings_by_key = {
            "annotation_list": PanelBinding(
                key="annotation_list",
                mode=LayoutMode.ANNOTATION,
                action=self.annotation_list_action,
                dock=self.annotation_list_dock,
                persist_visibility=True,
            ),
            "model_runner": PanelBinding(
                key="model_runner",
                mode=LayoutMode.ANNOTATION,
                action=self.model_runner_action,
                dock=self.model_runner_dock,
                persist_visibility=True,
            ),
            "model_evaluation": PanelBinding(
                key="model_evaluation",
                mode=LayoutMode.ANNOTATION,
                action=self.model_eval_action,
                dock=self.model_eval_dock,
                persist_visibility=True,
            ),
            "clinical_outcomes": PanelBinding(
                key="clinical_outcomes",
                mode=LayoutMode.ANNOTATION,
                action=self.clinical_action,
                dock=self.clinical_dock,
                persist_visibility=True,
            ),
            "irr_panel": PanelBinding(
                key="irr_panel",
                mode=LayoutMode.COMPARISON,
    ...
    def _panel_bindings(self) -> tuple[PanelBinding, ...]:
        return tuple(self._panel_bindings_by_key.values())

    def _panel_binding(self, key: str) -> PanelBinding:
        return self._panel_bindings_by_key[key]
    ...
    def _update_model_actions(self) -> None:
        model = self._model_workflows.current_model()
    ...
    def _update_toolbar_state(
        ...
        self.annotation_toolbar.set_selection_state(
            ...
            loop_active,
            has_ghost,
            editable=self._annotation_mutations_enabled(),
        )
        self.annotation_toolbar.set_speed(self.video_player.get_speed())

    def _annotation_mutations_enabled(self) -> bool:
        return self._layout_mode is LayoutMode.ANNOTATION and self.context is not None

    def _show_read_only_comparison_message(self) -> None:
        self.statusBar().showMessage(
            "Comparison mode is read-only for Session A annotations.",
            2500,
        )
    ...
    def _clear_session_state(self) -> None:
        ...
        self.show_comparison_action.setEnabled(False)
        self.show_comparison_action.setChecked(True)
        self._clear_loop_region_widgets()
    ...
    def _bind_context_state(self, context: WorkingContext) -> None:
        """Attach a loaded working context to the window state."""
        self.context = context
        self.session = context.session
        self.annotations = context.store
        self.context.loaded_models = dict(self._loaded_models)
        self.setWindowTitle(f"RIME - {context.session.name}")
        self.timeline.set_schema(context.schema)
        self.annotation_list_panel.set_schema(context.schema)

    def _restore_session_media(self) -> None:
        """Restore the session's media configuration into the player widgets."""
        if self.session and self.session.videos:
            self.video_player.load_videos(self.session.videos, self.session.session_dir)
        self._apply_playback_speed_default()

    def _restore_session_signal_preview(self) -> None:
        """Restore signal display state and persisted snap points."""
        if self.session is None:
            return
        try:
            self.timeline.signals.set_display_config(self._signal_display_entries())
            self.timeline.signals.set_combined_view(bool(self.session.signal_display_combined))
            self.timeline.set_snap_points(self.session.snap_points)
        except Exception as exc:
            self.timeline.signals.clear()
    ...
    def _restore_session_views(self) -> None:
        """Bind the restored store into the timeline, overview, and annotation list."""
    ...
    def _finalize_context_load(self, missing_models: list[str]) -> None:
        """Enable session actions and announce any model restore issues."""
        if self.session is None:
            return
        ...
        self.statusBar().showMessage(
            f"Loaded: {len(self.session.videos)} videos, {len(self.session.signals)} signals",
            5000,
        )

    def _clear_loop_region_widgets(self) -> None:
        """Clear ROI state across playback and overview widgets."""
        self.video_player.clear_loop()
        self.timeline.clear_loop_region()
        self.overview_strip.clear_loop_region()

    def _apply_loop_region_widgets(
        self,
        start_ms: float,
        end_ms: float,
        *,
        ...
        """Apply ROI state consistently across playback and overview widgets."""
        start, end = self.timeline.set_loop_region(start_ms, end_ms)
        self.overview_strip.set_loop_region(start, end)
    ...
    def _load_context(self, context: WorkingContext) -> None:
        ...
        self._bind_context_state(context)
        missing_models: list[str] = []
        self._suppress_session_persistence = True
        try:
            self._restore_session_media()
            self._restore_session_signal_preview()
            self._restore_session_views()
            missing_models = self._model_workflows.restore_session_models()
            self._restore_dock_layout_state()
            self._apply_layout(LayoutMode.ANNOTATION)
        finally:
            self._suppress_session_persistence = False
        self._finalize_context_load(missing_models)
    ...
    def _restore_checkpoint_snapshot(self, snapshot) -> None:
        ...
        if snapshot.loop_region is None:
            ...
            self._apply_loop_region_widgets(*snapshot.loop_region)
    ...
    def _on_load_model(self) -> None:
        self._model_workflows.load_model_from_dialog()

    def load_model_path(self, model_path: str | Path) -> bool:
        return self._model_workflows.load_model_path(model_path)

    def _register_loaded_model(self, package: CMFPackage) -> bool:
        return self._model_workflows.register_loaded_model(package)

    def _on_unload_model(self, model_name: str) -> None:
        self._model_workflows.unload_model(model_name)

    def _restore_session_models(self) -> list[str]:
        return self._model_workflows.restore_session_models()

    def _on_run_inference(self, model_name: str | None = None) -> None:
        self._model_workflows.run_inference(model_name)

    def _on_edit_model_settings(self, model_name: str | None = None) -> None:
        self._model_workflows.edit_model_settings(model_name)

    def _on_review_pending_ghosts(self, model_name: str | None = None) -> None:
        self._model_workflows.review_pending_ghosts(model_name)
    ...
    def _refresh_model_panel(self) -> None:
        ...
        targets_by_model = {
            model_name: self._model_workflows.build_output_mappings(
                model,
                self._model_workflows.current_model_settings(model_name),
            )
    ...
    def _on_matched_episode_store_changed(self, payload) -> None:
        if payload is None:
            self.timeline.set_matched_episode_store(None)
        else:
            store, label = payload
            self.timeline.set_matched_episode_store(store, label)
        self.timeline.refresh_annotations()

    def _on_toggle_panel(self, key: str, checked: bool | None = None) -> None:
        binding = self._panel_binding(key)
        if self._layout_mode is not binding.mode:
            return
        if checked is None:
            checked = binding.dock.isHidden()
        binding.dock.setVisible(checked)
        if checked:
            binding.dock.raise_()

    def _on_panel_dock_visibility_changed(self, key: str, visible: bool) -> None:
        binding = self._panel_binding(key)
        open_state = self._dock_is_open(binding.dock, visible=visible)
        self._set_action_checked(binding.action, open_state)
        if binding.persist_visibility:
            self._persist_panel_visibility(key, open_state)

    def _set_dock_visible(self, dock: QDockWidget, visible: bool, action=None) -> None:
        if action is not None:
            self._set_action_checked(action, visible)
        dock.setVisible(visible)

    @staticmethod
    def _dock_is_open(dock: QDockWidget, *, visible: bool | None = None) -> bool:
        if visible:
            return True
    ...
    def _apply_default_dock_widths(self) -> None:
        self.resizeDocks(
            [binding.dock for binding in self._panel_bindings()],
            [DOCK_DEFAULT_WIDTH] * len(self._panel_bindings()),
            Qt.Orientation.Horizontal,
        )

    def _ensure_right_dock_tabs(self) -> None:
        anchor = self.annotation_list_dock
        for binding in self._panel_bindings()[1:]:
            self.tabifyDockWidget(anchor, binding.dock)
    ...
    def _restore_dock_layout_state(self) -> None:
        ...
        try:
            ...
            self._ensure_right_dock_tabs()
            self._sync_panel_visibility_from_docks()
        finally:
            self._suppress_panel_visibility_persistence = False

    def _sync_panel_visibility_from_docks(self) -> None:
        if self.session is None:
            return
        for binding in self._panel_bindings():
            open_state = self._dock_is_open(binding.dock)
            self._set_action_checked(binding.action, open_state)
            if binding.persist_visibility:
                self.session.panel_visibility[binding.key] = open_state
    ...
    def _apply_layout(self, mode: LayoutMode) -> None:
        ...
        try:
            for binding in self._panel_bindings():
                active = (
                    comparison_active
                    if binding.mode is LayoutMode.COMPARISON
                    else mode is binding.mode
                )
                binding.action.setEnabled(active)
                visible = active and (
                    self._panel_visibility(binding.key) if binding.persist_visibility else True
                )
                self._set_dock_visible(binding.dock, visible, action=binding.action)
        finally:
            self._suppress_panel_visibility_persistence = False
    ...
    def _on_annotation_created(self, level: int, start_ms: float, end_ms: float) -> None:
        if not self._annotation_mutations_enabled():
            self._show_read_only_comparison_message()
            return
        if not self.context:
            return
        ...
        _annotation, violations = self.context.create_annotation(
            lane=lane_cfg.name,
            label=label,
            start_ms=start_ms,
            end_ms=end_ms,
        ...
        self._refresh_annotations_after_change()
    ...
    def _on_annotation_deleted(self, ann_id: str) -> None:
        ...
        self._refresh_annotations_after_change(clear_selection=True)
        self.statusBar().showMessage(f"Deleted {ann.label}", 2000)

    def _on_annotation_modified(self, _ann_id: str, _start_ms: float, _end_ms: float) -> None:
        self.overview_strip.set_annotations(self.annotations.all())
    ...
    def _refresh_annotations_after_change(
        self,
        *,
        selected_id: str | None = None,
        clear_selection: bool = False,
    ) -> None:
        """Refresh annotation-bound views after a store mutation."""
        if clear_selection:
            self.timeline.lanes.clear_selection()
        self._refresh_annotation_views()
        if selected_id is not None:
            self.timeline.select_annotation(selected_id)
        self._update_toolbar_state()
    ...
    def _on_edit_annotation(self, ann_id: str | None = None) -> None:
        ...
        self._refresh_annotations_after_change(selected_id=target_id)
    ...
    def _on_cut_annotation(self) -> None:
        ...
        self._refresh_annotations_after_change(selected_id=left.id)
    ...
    def _on_toggle_loop(self, checked: bool) -> None:
        ...
        if ann:
            ...
            self._apply_loop_region_widgets(start, end, seek_to_start=True)
    ...
    def _on_set_speed(self, rate: float) -> None:
        self._show_playback_speed(self.video_player.set_speed(rate))

    def _edit_preferences(self, *, initial_tab: str | None = None) -> None:
    ...
    def _on_edit_metadata(self) -> None:
        ...
        self.session.provenance.recording_relative_timing_verified = bool(
            updated["recording_relative_timing_verified"]
        )
        if any(
            (
                updated["subject_id"],
    ...
    def _show_shortcut_preferences(self) -> None:
        self._edit_preferences(initial_tab="Shortcuts")
    ...
    def _on_toggle_view_mode(self) -> None:
        self._toggle_video_display_mode_with_feedback()
    ...
    def _toggle_video_display_mode_with_feedback(self) -> None:
    ...
    def _show_playback_speed(self, actual: float) -> None:
        self.annotation_toolbar.set_speed(actual)
    ...
    def _on_delete_all_ghosts(self) -> None:
        ...
        self._remove_annotations_with_feedback(
            ghost_ids,
            checkpoint_label="Delete All Ghosts",
            empty_message="No ghost annotations to clear",
            success_message="Deleted {count} ghost annotations",
        )
    ...
    def _on_clear_all_annotations(self) -> None:
        ...
        self._remove_annotations_with_feedback(
            all_ids,
            checkpoint_label="Delete All Annotations",
            empty_message="No annotations to clear",
            success_message="Deleted {count} annotations",
        )
    ...
    def _clear_lane_annotations(
        ...
        *,
        ...
        self._remove_annotations_with_feedback(
            ...
            empty_message=f"No {'ghost ' if ghosts_only else ''}annotations on {lane_name} [{source_label}]",
            success_message=f"Cleared {{count}} {noun} on {lane_name} [{source_label}]",
        )

    def _remove_annotations_with_feedback(
        self,
        ...
        *,
        ...
        if not annotation_ids:
            ...
            return
        self._create_checkpoint(KIND_PRE_DESTRUCTIVE, checkpoint_label)
        count = self._remove_annotations_by_ids(annotation_ids)
        self.statusBar().showMessage(success_message.format(count=count), 2500)

    def _on_collapse_all(self) -> None:
        self._set_all_lane_groups_collapsed(True)

    def _on_expand_all(self) -> None:
        self._set_all_lane_groups_collapsed(False)

    def _set_all_lane_groups_collapsed(self, collapsed: bool) -> None:
        if hasattr(self, "timeline"):
            for name in self.timeline.lanes._group_state:
                self.timeline.lanes._group_state[name] = collapsed
            self.timeline.lanes._recalculate_height()
            self.statusBar().showMessage(
                "All lanes collapsed" if collapsed else "All lanes expanded",
                2000,
            )
    ...
    def keyPressEvent(self, event: QKeyEvent) -> None:
        ...
        if self._matches_shortcut(event, SPEED_DOWN):
            self._show_playback_speed(self.video_player.speed_down())
            event.accept()
            return

        if self._matches_shortcut(event, SPEED_UP):
            self._show_playback_speed(self.video_player.speed_up())
            event.accept()
            return
