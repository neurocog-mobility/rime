...
def test_serialization_roundtrip(tmp_path: Path) -> None:
    ...
    assert loaded_ann.confidence_type == "not_recorded"
...
def test_old_annotation_container_is_rejected(tmp_path: Path) -> None:
    import pytest

    path = tmp_path / "annotations.json"
    path.write_text('{"version":"1.0","session":{},"annotations":[]}')
    with pytest.raises(ValueError, match="native annotation"):
        AnnotationStore.load(path)
