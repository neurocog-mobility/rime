...
def _store() -> AnnotationStore:
    ...
    store.add(
        Annotation(id="a1", lane="FOG", label="FOG", start_ms=100.0, end_ms=200.0, source="manual")
    )
    return store
...
def test_pre_destructive_checkpoints_are_capped(tmp_path: Path) -> None:
    ...
    for index in range(12):
        create_checkpoint(session_dir, _store(), label=f"Clear {index}", kind=KIND_PRE_DESTRUCTIVE)
    checkpoints = [
        entry for entry in list_checkpoints(session_dir) if entry.kind == KIND_PRE_DESTRUCTIVE
    ]
...
def test_restore_guard_checkpoints_have_separate_small_cap(tmp_path: Path) -> None:
    session_dir = tmp_path / "session"
    create_checkpoint(session_dir, _store(), label="Session opened", kind=KIND_WORKSPACE_OPEN)
    create_checkpoint(session_dir, _store(), label="Manual", kind=KIND_MANUAL)
    for index in range(3):
        create_checkpoint(
            session_dir, _store(), label=f"Before restore {index}", kind=KIND_RESTORE_GUARD
        )
    checkpoints = list_checkpoints(session_dir)
    restore_guards = [entry for entry in checkpoints if entry.kind == KIND_RESTORE_GUARD]
    assert len(restore_guards) == 2
    assert any((entry.kind == KIND_WORKSPACE_OPEN for entry in checkpoints))
