...
def test_timeline_comparison_respects_source_filters() -> None:
    ...
    filtered_primary = timeline.lanes._comparison_annotations(
        timeline.lanes._store, "FOG", source=timeline.lanes._primary_source_filter
    )
    filtered_comparison = timeline.lanes._comparison_annotations(
        timeline.lanes._comparison_store, "FOG", source=timeline.lanes._comparison_source_filter
    )
...
def test_timeline_shows_source_labels_for_both_compare_rows() -> None:
    ...
    comparison.add(
        Annotation(
            id="b1", lane="FOG", label="FOG", start_ms=120.0, end_ms=280.0, source="model:demo"
        )
    )
    timeline.set_store(primary)
    timeline.set_comparison_store(comparison)
...
def test_timeline_supports_matched_episode_row_and_overlays() -> None:
    ...
    matched.add(
        Annotation(
            id="m1", lane="FOG", label="FOG", start_ms=110.0, end_ms=290.0, source="matched:average"
        )
    )
    timeline.set_store(primary)
    timeline.set_comparison_store(comparison)
    ...
    level = timeline.lanes._lane_name_to_level("FOG")
    assert level is not None
    assert timeline.lanes._lane_sources(level) == [
        SESSION_A_SOURCE,
        COMPARISON_SOURCE,
        MATCHED_MODE_SOURCE,
    ]
